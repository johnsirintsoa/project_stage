...
export default {
  ...
  created() {
    const id_autorite_enfant = window.location.pathname.split('/')[window.location.pathname.split('/').length-1]
    this.audience.receiver = parseInt(id_autorite_enfant)

    const profil = JSON.parse(sessionStorage.getItem('administrateur'));
    this.audience.sender = profil.autorite_enfant.id
    // console.log(parseInt(this.audience.receiver))
  },
  methods:{
    async all_actual_events(){
        // const profil = JSON.parse(sessionStorage.getItem('administrateur'));
        // console.log(profil.autorite.id_autorite_enfant)
        
        return await actual_events_autorite(this.audience.receiver)
    ...
    async add_event(){
      const audience_event = {
        ...
        // id_demande_stage: 35,
        id_autorite_enfant_receiver: this.audience.receiver,
      ...
      const response =  await DemandeAudience.add_event_autorite(audience_event)
      console.log(audience_event)
      console.log(response)
      if(response.code == 'ER_BAD_FIELD_ERROR'){
        swal("Audience non enregistrée", "Vous avez un erreur", "error");
      }
        ...
        <div class='demo-app'>
          <div v-if="is_clicked" class='demo-app-sidebar'>
              <div class='demo-app-sidebar-section'>
                  ...
                  <form class="form-audience" novalidate>
                      <ul>
