    ...
    export default {
      ...
      methods: {
        ...
        async all_actual_events(){
          const ses = JSON.parse(sessionStorage.getItem('autorite'))
          console.log(ses.id_autorite_enfant)
          const events = await faire_audience(ses.id_autorite_enfant,this.audience.direction)
          return events
        },
        ...
        async list_heure_dispo(id_dm_aud_autorite_date_heure_dispo){
          ...
          hd.forEach(element => {
            if(id_dm_aud_autorite_date_heure_dispo){
